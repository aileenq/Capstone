...
## Auth Header
def validate_auth_header(auth_header_values):
    # check if it's bearer token
    if auth_header_values[0].lower() != 'bearer':
        raise AuthError({
            'code': 'invalid_header_bearer_token',
            'description': 'Authorization header must start with "Bearer".'
        }, 401)
    # invalid format
    elif len(auth_header_values) == 1:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'jwt Token not found.'
        }, 401)
    elif len(auth_header_values) > 2:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header invalid format.'
        }, 401)
...
def get_token_auth_header():
    ...
    validate_auth_header(auth_header_values)
    token = auth_header_values[1]
    return token
...
def verify_decode_jwt(token):
    ...
    if 'kid' not in unverified_header:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization header is missing kid.'
        }, 401)
